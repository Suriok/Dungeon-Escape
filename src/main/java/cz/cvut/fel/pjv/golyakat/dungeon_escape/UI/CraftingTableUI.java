...
public class CraftingTableUI {
    ...
    public CraftingTableUI(gamePanel gp) {
        ...
        this.craftingSlots = new ChestInventoryManager.ItemData[3]; // For Key1, Key2, Key3
    ...
    // CraftingTableUI.java
    public void craftSilverKey() {

        // --- 1. собираем информацию о том, какие части лежат в ячейках ---
        ...
        for (ChestInventoryManager.ItemData slot : craftingSlots) {
            ...
            switch (slot.getName()) {
                ...
                default      -> { /* чужие предметы игнорируем */ }
            }
        }
        ...
        // --- 4. чистим слоты верстака ---
        for (int i = 0; i < craftingSlots.length; i++) {
            craftingSlots[i] = null;
        }

        // --- 5. вычитаем потраченные Key-части из инвентаря игрока (если у него были дубликаты) ---
        ...
        for (String part : parts) {
            for (int i = 0; i < gp.player.getInventory().size(); i++) {
                ...
                if (it.getName().equals(part)) {
                    ...
                    break;               // уменьшили ровно на 1 — выходим
                }
            }
        }
